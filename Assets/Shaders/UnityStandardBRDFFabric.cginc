...
#ifndef UNITY_STANDARD_BRDF_FUR_INCLUDED
#define UNITY_STANDARD_BRDF_FUR_INCLUDED
...
inline half FabricFresnelLerp(half nv, half scale)
{
	half t0 = Pow4(1 - nv);
	half t1 = 0.4 * (1 - nv);
...
inline float FabricGGXTerm(float NdotH, float roughness)
{
	return 0.96 * pow(1 - NdotH, 2) + 0.057;
}
...
#define UNITY_HANDLE_CORRECTLY_NEGATIVE_NDOTV 0
	...
	float VxD = roughness > 0.99 ? 1 * GGXTerm(nh, roughness) : V * D;

    float specularTerm = VxD * UNITY_PI; // Torrance-Sparrow model, Fresnel is applied later
...
#   endif
	...
	half3 color = diffColor * (gi.diffuse + light.color * diffuseTerm)
		...
		+ _FabricColor * (nl * 0.5 + 0.5) * FabricFresnelLerp(nv, _FabricScale);
