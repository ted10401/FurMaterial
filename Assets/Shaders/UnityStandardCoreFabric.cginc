...
#ifndef UNITY_STANDARD_CORE_INCLUDED_FUR
#define UNITY_STANDARD_CORE_INCLUDED_FUR

#include "UnityCGFabric.cginc"
#include "UnityShaderVariables.cginc"
#include "UnityStandardConfigFabric.cginc"
#include "UnityStandardInputFabric.cginc"
#include "UnityPBSLightingFabric.cginc"
#include "UnityStandardUtilsFabric.cginc"
#include "UnityGBuffer.cginc"
#include "UnityStandardBRDFFabric.cginc"
...
VertexOutputForwardBase vertForwardBaseFur(VertexInput v, half FUR_OFFSET)
{
	UNITY_SETUP_INSTANCE_ID(v);
	VertexOutputForwardBase o;
	UNITY_INITIALIZE_OUTPUT(VertexOutputForwardBase, o);
	...
	o.tex = TexCoords(v);
	o.tex_fur = TRANSFORM_TEX(v.uv0, _FurTex);
	o.eyeVec.xyz = NormalizePerVertexNormal(posWorld.xyz - _WorldSpaceCameraPos);
	float3 normalWorld = UnityObjectToWorldNormal(v.normal);
#ifdef _TANGENT_TO_WORLD
	float4 tangentWorld = float4(UnityObjectToWorldDir(v.tangent.xyz), v.tangent.w);
...
half4 fragForwardBaseInternalFur(VertexOutputForwardBase i, half FUR_OFFSET)
{
	UNITY_APPLY_DITHER_CROSSFADE(i.pos.xy);
	...
	FUR_OFFSET *= 0.05;
	c.rgb -= pow(1 - FUR_OFFSET, 3) * _FurShading;

	float3 worldNormal = normalize(s.normalWorld);
	float3 worldView = normalize(_WorldSpaceCameraPos.xyz - s.posWorld);
	float fresnel = 1 - max(0, dot(worldNormal, worldView));
	float rimLight = fresnel * (FUR_OFFSET * FUR_OFFSET + 0.04);
	c.rgb += _RimColor * pow(rimLight, _RimPower);

	fixed4 furCol = tex2D(_FurTex, i.tex_fur);
